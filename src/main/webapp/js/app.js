...
app.controller('AppCtrl', ['$scope', '$mdBottomSheet', '$mdSidenav', '$mdDialog', '$location', '$mdToast', function ($scope, $mdBottomSheet, $mdSidenav, $mdDialog, $location, $mdToast) {
    ...
    $scope.showId = function (rowId) {
        $mdToast.show(
            $mdToast.simple()
                .content('Row id: ' + rowId + ' is about to be deleted')
                .hideDelay(3000)
        );
    }
    ...
    $scope.showAdd = function (ev) {
        switch ( $scope.title ) {
            ...
            default :
                $mdToast.show(
                $mdToast.simple()
                    .content('Add Button clicked, but no dialog has been written for [' + $scope.title + '] yet...')
                    .hideDelay(5000)
                );
        }
    };

    $scope.showAddPersonDialog = function (event) {
        $mdDialog
            .title('Add Person')
            .targetEvent(event)
            .ok('Save')
...
app.controller('peopleController', ['$scope', 'peopleFactory', '$mdToast', '$mdDialog', function ($scope, peopleFactory, $mdToast, $mdDialog) {
    ...
    $scope.selectedRowCallback = function (rows, event) {
        if (rows.length > 0) {
            // dialog settings
            var confirm = $mdDialog.confirm()
                .title('Do you want to edit this Person\'s information?')
                .targetEvent(event)
                .ok('Yes')
                .cancel('No');
            ...
            $mdDialog.show(confirm).then(
                ...
                function () {
                    ...
                    $mdToast.show(
                        $mdToast.simple()
                            .content('Edit person dialog will open for ID [' + rows + ']')
                            .hideDelay(5000)
                    );
                },
                // 'No' button
                function () {
                    $scope.reloadPeopleTable();
                })
        } else {
            return;
        }
